...
unicode切片动态数组 创建unicode切片动态数组(int 初始容量) {
    ...
    // 创建数据区
    数组.数据区 = (unicode切片 *)malloc(初始容量*sizeof(unicode切片));
    return 数组;
}
/******************************************************************************
 * 初始化unicode切片的动态数组
...
void unicode切片动态数组_初始化(unicode切片动态数组 * 数组指针, int 初始容量) {
    if (数组指针 != NULL) {
        数组指针->容量 = 初始容量;
        数组指针->长度 = 0;
        数组指针->数据区 = (unicode切片 *)malloc(初始容量*sizeof(unicode切片));
    }
}
/******************************************************************************
 * 释放unicode切片动态数组的数据区
...
void unicode切片动态数组_重置(unicode切片动态数组 * 数组指针) {
    ...
    unicode切片动态数组_初始化(数组指针, 8);  //初始分配64元素的空间
}
/******************************************************************************
...
void unicode切片动态数组_扩容(unicode切片动态数组 * 数组指针, int 期望容量) {
    ...
    新数据区指针 = realloc(数组指针->数据区, 新容量*sizeof(unicode切片));
    // realloc失败检查
    if (新数据区指针 == NULL) {
        perror("切片扩容内存分配失败！\n");
    }
